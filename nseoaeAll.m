function [oae, numOK] = nseoaeAll(params,y)
...
%   
%   adapted by Vencovsky to nseoaeAll(params,y)
%   nsoaeAll(params, yn) estimates the amplitude and phase of N components of
...
idx = 1;
GainMicA = params.GainMicA; % gain of the microphone in OAE probe
    ...
    switch params.component(i).type,
        ...
        case {'logsweep'},        
            ...
            % The version results is slightly out of phase            
             ...
             % This is default version in oaetoolbox
...
%     ttvec = (1:(nfilt))*dt + t1;
    ...
    Yn = abs(fft(Wvec .* y(i1:i2))/nfilt); % fft for new signal to estimate background noise
    ...
    NoiseFloordB = 20*log10(abs(giveTrueSPL(mean(fx(IdNoise1:IdNoise2)),NoiseFloor,GainMicA,fs,1,fx))/(sqrt(2)*2e-5));
    
    if NoiseFloordB<0 % for smaller noise than 10 dB SPL include this frame
%        if nargin<3
%            y(i1:i2) = yn(i1:i2);
%            yp(:,istep) = y(i1:i2);
%        else
