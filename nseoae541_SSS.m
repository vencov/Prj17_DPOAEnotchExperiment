function [oae, yp, numOK] = nseoae541_SSS(params,yn,yp,numOK)
...
%
%   NSEOAE(Y, PARAMS) estimates the amplitude and phase of N components of
%   the signal contained in Y.
%
%   PARAMS is a structure containing N components to be estimated.
%   EX.
%        y=0;
%        load('LSWrec_s002_190123_141643_F2b_1500_F2e_2500_L1_60_L2_60F2F1_120_Right_01.mat')
%        y=y+recordedSignal;
% %        plot(y(:,1))
%        load('LSWrec_s002_190123_141643_F2b_1500_F2e_2500_L1_60_L2_60F2F1_120_Right_05.mat')
%        y=y+recordedSignal;
%        load('LSWrec_s002_190123_141643_F2b_1500_F2e_2500_L1_60_L2_60F2F1_120_Right_09.mat')
%        y=y+recordedSignal;
%        load('LSWrec_s002_190123_141643_F2b_1500_F2e_2500_L1_60_L2_60F2F1_120_Right_13.mat')
%        y=y+recordedSignal;
%        load('LSWrec_s002_190123_141643_F2b_1500_F2e_2500_L1_60_L2_60F2F1_120_Right_17.mat')
%        y=y+recordedSignal;
%        load('LSWrec_s002_190123_141643_F2b_1500_F2e_2500_L1_60_L2_60F2F1_120_Right_21.mat')
% %        hold on; plot(y(:,1))
%        
...
%       params.nskip = 6200;                         % ?samples to skip, default = 0 (e.g. due to system delay, propogation delay)
      % load ../Results/s004/r111l/LSWrec_s004_191030_164823_F2b_1000_F2e_4000_L1_50_L2_50F2F1_111_Left_01.mat     
       
      % fcutoff =300; % cutoff freq of 2order high pass butterworth filter
      % [b,a] = butter(4,fcutoff/(fs/2),'high');
      % yn = (filter(b,a,recordedSignal(:,1)))';
       %yp = [];
       
       
       %f2f1 = 1.11;
%         params.component(1).f1 = 2*F2start/f2f1-F2start; % logsweep start frequency  (for 'tone' type, only f1 is necessary)
%         params.component(1).f2 = 2*F2stop/f2f1-F2stop; % logsweep end frequency    (for 'tone' type, only f1 is necessary)
% 
%         D = 4170;
%         params.nskip = D;% 1.26e4;                         % ?samples to skip, default = 0 (e.g. due to system delay, propogation delay)
...
%         params.T = log2(F2stop/F2start)/OctperS;     % poustelo se rychlosti 0.5 oktavy/s                               % 6 second sweep
%         params.nfilt = 5512;                        % window size (125ms, recommended for 0.5 oct/s)
%        %params.nfilt = 22050;                       % window size (500ms, recommended for 0.125 oct/s)
%        params.nstep = 51;                          % step size
%        params.component(1).type = 'logsweep';     
%        
%         params.windowType = 2;                      % window function (-1 = rectangular, 0 = welch, 1 = hamming, 2 = hann [default], 3 = parzen, 4 = blackman)
%        params.fs = 44100;                          % sampling rate (default = 44100);
%
...
%             Phi0(i) = (2*pi) * f1 * T/log(f2/f1);
            Phi{i} = @(t) 2*pi*f1*gamma*exp(t/gamma);
            PhiF{i} = @(t) f1.*exp(t/gamma);
...
%     ttvec = (1:(nfilt))*dt + t1;
    ...
    Fbot = pfFreq(1)*(1 - 1/(2-params.f2f1) + 1);
    Fup = pfFreq(1);
    Fdif = Fup - Fbot;
    dFdif = 0.3*Fdif;
    IdNoise1 = find(fx>=dFdif + (pfFreq(1)*(1 - 1/(2-params.f2f1) + 1)),1,'first'); % find the lowest freq edge
    IdNoise2 = find(fx>=pfFreq(1) - dFdif,1,'first'); % find the highest freq edge;
    IdNoise3 = find(fx>=dFdif + pfFreq(1),1,'first'); % find the lowest freq edge
    IdNoise4 = find(fx>=pfFreq(1)/(2-params.f2f1) - dFdif,1,'first'); % find the highest freq edge;
    IdNoise5 = find(fx>=dFdif + pfFreq(1)/(2-params.f2f1),1,'first'); % find the lowest freq edge
    IdNoise6 = find(fx>=params.f2f1*pfFreq(1)/(2-params.f2f1) - dFdif,1,'first'); % find the highest freq edge;
    IdNoise7 = find(fx>=dFdif + params.f2f1*pfFreq(1)/(2-params.f2f1),1,'first'); % find the lowest freq edge
    IdNoise8 = find(fx>=pfFreq(1)/(2-params.f2f1)*(2*params.f2f1 -1) - dFdif,1,'first'); % find the highest freq edge;
    %IdNoise2 = find(fx>=pfFreq(1)+params.dFnoise,1,'first'); % find the lowest freq edge
%     NoiseFloor = max([Yn(IdNoise1:IdNoise2) Yn(IdNoise3:IdNoise4) Yn(IdNoise5:IdNoise6) Yn(IdNoise7:IdNoise8)]); % estimate noise in the signal
    NoiseFloor1 = max(Yn(IdNoise1:IdNoise2));
    NoiseFloor2 = max(Yn(IdNoise3:IdNoise4));
    NoiseFloor3 = max(Yn(IdNoise5:IdNoise6));
    NoiseFloor4 = max(Yn(IdNoise7:IdNoise8)); % estimate noise in the signal
    NoiseFloordB1 = 20*log10(abs(giveTPaLinux(mean(fx(IdNoise1:IdNoise2)),NoiseFloor1,GainMicA))/(sqrt(2)*2e-5));
    NoiseFloordB2 = 20*log10(abs(giveTPaLinux(mean(fx(IdNoise3:IdNoise4)),NoiseFloor2,GainMicA))/(sqrt(2)*2e-5));
    NoiseFloordB3 = 20*log10(abs(giveTPaLinux(mean(fx(IdNoise5:IdNoise6)),NoiseFloor3,GainMicA))/(sqrt(2)*2e-5));
    NoiseFloordB4 = 20*log10(abs(giveTPaLinux(mean(fx(IdNoise7:IdNoise8)),NoiseFloor4,GainMicA))/(sqrt(2)*2e-5));
%     NoiseFloordB = 20*log10(abs(giveTPaLinux(mean(fx(IdNoise1:IdNoise2)),NoiseFloor,GainMicA,fs,1,fx))/(sqrt(2)*2e-5));
%     YnVect(:,istep) = Yn;
    NoiseFloordB = max([NoiseFloordB1 NoiseFloordB2 NoiseFloordB3 NoiseFloordB4]);
    if NoiseFloordB<50 % for smaller noise than 10 dB SPL include this frame
       if nargin<3
           y(i1:i2) = yn(i1:i2);
           yp(:,istep) = y(i1:i2);
       else
